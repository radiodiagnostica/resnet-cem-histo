#!/usr/bin/env python3
# ------------------------------------------------------------
#  Hormone-receptor (HR) prediction – v6.2
#     calibrated probabilities  +  flexible threshold tuning
# ------------------------------------------------------------
...
import matplotlib, matplotlib.pyplot as plt, seaborn as sns; matplotlib.use("Agg")
...
@dataclass
class CFG:
    data_dir:str='./';      tag:str='resnet18_rep';   epochs:int=30;   bs:int=4
    lr:float=1e-5;          wd:float=5e-4;            patience:int=7
    img_sz:tuple=(224,224); n_classes:int=2;          best_metric:str='pr_auc'
    pos_cls:int=1;          seed:int=86;              boot:int=1_000;  ci:float=.95
    loss_weights:tuple=(1.,2.5)

    # ── NEW threshold-tuning options ────────────────────────
    goal:str='f1'          # 'fixed'|'f1'|'fbeta'|'bal_acc'|'youden'
    beta:float=2.0         # used when goal=='fbeta'
    min_recall:float=None  # e.g. 0.80
    fixed_th:float=0.5     # used when goal=='fixed'
CFG=CFG()
...
def build_transforms():
    ...
    val=transforms.Compose([transforms.Grayscale(3),transforms.Resize(256),
                            transforms.CenterCrop(CFG.img_sz),transforms.ToTensor(),norm])
    return norm,aug,val,{'train':aug,'val':val,'external_val':val}
norm, aug, val_tf, dtrans = build_transforms()

# ------------------------------- METRICS -------------------------------------------
def _pr_auc(y,p): pr,rc,_=precision_recall_curve(y,p,pos_label=CFG.pos_cls); return auc(rc,pr)
    ...
    for ep in range(1,CFG.epochs+1):
        for phase in ['train','val']:
            ...
            for x,l in dls[phase]:
                ...
                y.extend(l.cpu().numpy()); p.extend(probs); ŷ.extend(o.argmax(1).cpu().numpy())
...
class ModelWithTemperature(nn.Module):
    ...
    def set_temperature(self,loader):
        ...
        def _cl(): optT.zero_grad(); loss=nll(logits/self.temperature,labels); loss.backward(); return loss
        optT.step(_cl)
        print(f'   [Calibration] temperature = {self.temperature.item():.3f}')
        return self

# ------------------------------- THRESHOLD TUNER -----------------------------------
def tune_threshold(y,p,goal='f1',beta=1,min_recall=None):
    cand=np.unique(np.r_[0,(p[:-1]+p[1:])/2,1])
    best,bth=-np.inf,.5
    for t in cand:
        ŷ=(p>=t).astype(int)
        prec,rec,f1,_=precision_recall_fscore_support(y, ŷ,average='binary',zero_division=0)
        if min_recall and rec<min_recall: continue
        if   goal=='f1':      score=f1
...
def eval_phase(model,dl,th=.5):
    ...
    y,p=np.asarray(y),np.asarray(p); ŷ=(p>=th).astype(int)
    ...
    for i in range(CFG.boot):
        ...
        yb,pb=y[idx],p[idx]; ŷb=(pb>=th).astype(int); mb=_all_metrics(yb, ŷb, pb)
...
# ------------------------------- PLOTS & CAM (unchanged) ---------------------------
def plot_hist(h):
    e=range(1,len(h['tr_loss'])+1); plt.figure(figsize=(10,4))
...
def cm_plot(y,ŷ,name):
    ...
    sns.heatmap(cm,annot=True,fmt='d',cmap='Blues',
                xticklabels=['Pred HR+','Pred HR-'],yticklabels=['True HR+','True HR-'])
...
def save_heatmaps(model,dataset,n=50,save_dir='heatmaps'):
    ...
    blk=model.layer4[-1]; acts,grads=[],[]
    def fw(_,__,out): acts.append(out.detach()); out.register_hook(lambda g:grads.append(g))
    h=blk.register_forward_hook(fw); model.eval()
    for idx in random.sample(range(len(dataset)),min(n,len(dataset))):
        ...
        logits[0,CFG.pos_cls].backward(); A,G=acts[0],grads[0]; w=G.mean(dim=(2,3),keepdim=True)
        cam=(w*A).sum(1,keepdim=True).relu()
        cam=F.interpolate(cam,(img.size[1],img.size[0]),mode='bilinear',align_corners=False)[0,0].cpu().numpy()
        cam=(cam-cam.min())/(cam.max()+1e-9)
        plt.figure(figsize=(3,3)); plt.imshow(img); plt.imshow(cam,cmap='jet',alpha=.5); plt.axis('off')
        plt.tight_layout(pad=0); plt.savefig(f'{save_dir}/{os.path.basename(pth)}',dpi=300); plt.close()
...
def main():
    ...
    for k,v in asdict(CFG).items(): p.add_argument(f'--{k}',type=type(v),default=v)
    CFG=CFG.__class__(**vars(p.parse_args())); print('CONFIG:',CFG)

    set_seed(CFG.seed); norm, aug, val_tf, dtrans=build_transforms()
    dls,imgs=get_dls(); sizes={k:len(v) for k,v in imgs.items()}

    base=models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
    base.fc=nn.Linear(base.fc.in_features,CFG.n_classes); base=_compile(base.to(device))
    base,hist=train(base,dls,sizes); plot_hist(hist)

    model=ModelWithTemperature(base).to(device).set_temperature(dls['val'])

    # ---------------- threshold choice ------------------------------------------
    if CFG.goal=='fixed':
        th=CFG.fixed_th
        print(f'Using fixed threshold {th:.3f}')
    else:
        yv,pv=get_preds(model,dls['val'])
        th=tune_threshold(yv,pv,goal=CFG.goal,beta=CFG.beta,min_recall=CFG.min_recall)
    ...
    save_heatmaps(model.model, imgs['val'])
